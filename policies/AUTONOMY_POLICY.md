...
10. SAME-RUN CHAINING: after independent PASS, recompute dependencies immediately. If another non-duplicate task is eligible and all execution, validation, checkpoint/rollback and protection requirements are satisfied, continue to that task in the same controller run. Do not wait for the next scheduled trigger solely because a prior task completed.
...
17. REPOSITORY HYGIENE IS PART OF TASK COMPLETION. All agents MUST follow `policies/REPOSITORY_HYGIENE_POLICY.md`. Task-created temporary, failed, duplicate, superseded and otherwise disposable artifacts must be removed or avoided before final PASS when safe; preserve only concise evidence, required current artifacts, validation/provenance material and rollback/checkpoint requirements. Do not defer cheap task-local cleanup into later Codex or maintenance work.
18. Reviewer MUST include repository-hygiene verification in Definition of Done. A task with known disposable task-created artifacts is not COMPLETE unless retention is explicitly justified or cleanup is unsafe/human-gated.
