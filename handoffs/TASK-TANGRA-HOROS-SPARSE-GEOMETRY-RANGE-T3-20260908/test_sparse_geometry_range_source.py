...
CAL=CameraCalibration("HQ_PROVISIONAL_TEST", (2028,1520), 15756.86,15848.54,1014.0,760.0,
                      relative_scale_sigma=0.001, coordinates_undistorted=False,
                      production_verified=False, provenance=(("fixture","SYNTHETIC"),))
...
def geom(z=20.0, projection=1.0, invalid_nt=False, transform_valid=True, production_transform=False):
    hw=(W*projection/z)/2; hl=(L*projection/z)/2
    pts={"CENTER":(0.0,0.0),"LEFT_SILHOUETTE":(-hw,0.0),"RIGHT_SILHOUETTE":(hw,0.0),"NOSE":(0.0,-hl),"TAIL":(0.0,hl)}
    out=[]
    for name,(nx,ny) in pts.items():
        valid=True; x=CAL.cx+nx*CAL.fx; y=CAL.cy+ny*CAL.fy
...
class TestSparseRange(unittest.TestCase):
    ...
    def test_04_insufficient_valid_spans(self):
        p=profile(include_diag=False); p=replace(p,spans=(p.spans[0],replace(p.spans[1],correspondence_verified=False)))
    ...
    def test_07_disagreement_fail_closed(self):
        p=profile(include_diag=False); p=replace(p,spans=(p.spans[0],replace(p.spans[1],length_m=L*1.8)))
        o=SparseGeometryRangeSource(CAL,p).estimate(geom())
    ...
    def test_09_near_zero_image_span(self):
        g=geom(); pts=list(g.points); l=next(p for p in pts if p.point_type=="LEFT_SILHOUETTE")
    ...
    def test_10_mild_synthetic_foreshortening_explicit(self):
        q=0.94
        o=SparseGeometryRangeSource(CAL,profile(projection=q,orient_sigma=.02)).estimate(geom(25.0,projection=q))
    ...
    def test_11_range_uncertainty_grows_with_point_error_at_far_range(self):
        src=SparseGeometryRangeSource(CAL,profile(),RangeConfig(point_sigma_px=1.2))
        self.assertGreater(src.estimate(geom(50.0)).sigma_m,src.estimate(geom(8.0)).sigma_m)
    def test_12_repeatability(self):
        src=SparseGeometryRangeSource(CAL,profile()); g=geom(20.0); vals=[src.estimate(g) for _ in range(10)]
    ...
    def test_15_nonzero_distortion_requires_undistorted_coordinates(self):
        ...
        self.assertEqual(o.validity,RangeValidity.INVALID); self.assertIn("distortion_not_supported",o.error)
    ...
    def test_17_existing_range_shadow_comparison(self):
        ...
        c=compare_with_existing(ExistingRangeObservation("MONOCULAR_CLASS_SIZE",21.0,2.0,.7,True),sparse)
    ...
    def test_18_candidate_exposes_calibrated_pixel_span_and_full_provenance(self):
        ...
        self.assertEqual(keys["transform_version"],"1")
