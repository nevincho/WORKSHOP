...
class SparseGeometryRangeSource:
    ...
    def estimate(self, geometry: CalibratedGeometryInput) -> GeometryRangeObservation:
        ...
        try:
            ...
            if len(groups) < self.config.min_independent_candidates:
                return self._finish_invalid(geometry, t0, tuple(candidates), "insufficient_independent_evidence")

            # Prevent correlated spans from multiplying weight: retain best-information candidate per group.
            representatives = []
            ...
            if len(representatives) == 2:
                ...
                if rel > self.config.max_relative_pair_disagreement:
                    return self._finish_invalid(geometry, t0, tuple(candidates), "candidate_disagreement")
            ...
            if not np.all(np.isfinite(weights)) or float(weights.sum()) <= 0:
                return self._finish_invalid(geometry, t0, tuple(candidates), "invalid_fusion_weights")
    ...
    def _candidate(self, span: PhysicalSpanSpec, points: dict[str, CalibratedPoint],
        ...
        base_prov = (("profile_span", span.provenance), ("span_id", span.span_id),
                     ("source_geometry_ref", geometry.source_geometry_ref),
                     ("calibration_id", self.calibration.calibration_id),
                     ("transform_id", geometry.transform_id),
                     ("transform_version", geometry.transform_version))
        ...
        du = float(b.x - a.x)
        dv = float(b.y - a.y)
        nx = du / self.calibration.fx
        ny = dv / self.calibration.fy
        pixel_span = hypot(du, dv)
        q = hypot(nx, ny)
        if (not isfinite(q)) or q < self.config.min_normalized_image_span:
            return self._reject(span, "near_zero_image_span", base_prov, pixel_span=pixel_span, q=q)
        ...
        if orientation_conf < span.min_orientation_confidence:
            return self._reject(span, "orientation_confidence_too_low", base_prov, pixel_span=pixel_span, q=q)
        s_eff = span.length_m * span.orientation_projection_factor
        z = s_eff / q
        if not isfinite(z) or z <= 0:
            return self._reject(span, "impossible_range", base_prov, pixel_span=pixel_span, q=q)

        # q = sqrt((du/fx)^2 + (dv/fy)^2); independent endpoint sigma in each axis.
        ...
        rel_phys = span.sigma_m / span.length_m
        rel_q = sigma_q / q
        rel_orient = span.orientation_factor_sigma / span.orientation_projection_factor
        rel_cal = self.calibration.relative_scale_sigma
        rel_transform = geometry.transform_relative_scale_sigma
        rel_sigma = sqrt(rel_phys ** 2 + rel_q ** 2 + rel_orient ** 2 + rel_cal ** 2 + rel_transform ** 2)
        ...
        if confidence < self.config.min_candidate_confidence:
            return self._reject(span, "candidate_confidence_too_low", base_prov, pixel_span=pixel_span, q=q)
        return RangeCandidate(
            span.span_id, span.independence_group, span.point_a, span.point_b, pixel_span, q,
    ...
    def _robust_acceptance(self, candidates: list[RangeCandidate]) -> list[RangeCandidate]:
        ...
        # Robust center is computed from one best-information representative per
        # independence group so correlated spans cannot dominate the median.
        reps = [min(cg, key=lambda c: float(c.sigma_m) / max(c.confidence, 1e-9))
    ...
    def _reject(self, span: PhysicalSpanSpec, reason: str,
                provenance: Tuple[Tuple[str, str], ...], pixel_span: Optional[float] = None,
                q: Optional[float] = None) -> RangeCandidate:
        return RangeCandidate(span.span_id, span.independence_group, span.point_a, span.point_b,
                              pixel_span, q, span.length_m, span.sigma_m, None, None, 0.0, False, False, reason, provenance)

    def _finish_invalid(self, g: CalibratedGeometryInput, t0: int,
                        candidates: Tuple[RangeCandidate, ...], error: str) -> GeometryRangeObservation:
        return self._invalid(g, t0, candidates, error)
